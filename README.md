...
2. **Variable Without Type Rule (`var-no-type`)**
   ...
   - **Exception**: Allows declarations where the type is explicit in the value: `var x = []int{1, 2}`, `var a = make([]int, 0)`, `var b = x.(int)`, unambiguous literals: `var s = "hello"`, `var b = true`, or function literals: `var f = func(int) error { ... }`
